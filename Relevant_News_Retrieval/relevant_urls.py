...
GDELT_DAILY_DIR = Path("data/interim/gdelt_event_context_daily")
...
def _load_gdelt_latlon(target_date: str) -> pd.DataFrame:
    """Return a url_normalized → (actiongeo_lat, actiongeo_lon, sqldate, dateadded) lookup
    from the enriched GDELT file for target_date, or an empty DataFrame if not found."""
    ...
    if not enriched.exists():
        ...
        return pd.DataFrame(columns=["url_normalized", "actiongeo_lat", "actiongeo_lon", "sqldate", "dateadded"])
    cols = ["url_normalized", "actiongeo_lat", "actiongeo_lon"]
    all_cols = pd.read_csv(enriched, nrows=0).columns.tolist()
    for extra in ("sqldate", "dateadded"):
...
def main(target_date: str, top_k: int = 0, force: bool = False):
    ...
    for geo_col in ["actiongeo_lat", "actiongeo_lon", "sqldate", "dateadded"]:
