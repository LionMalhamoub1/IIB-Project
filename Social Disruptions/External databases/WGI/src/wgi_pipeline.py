...
from wgi_client import WGIClient, WGIClientConfig


START_YEAR = 2017
END_YEAR = 2025

WGI_ROOT = Path(__file__).resolve().parents[1]
REGISTRY_PATH = WGI_ROOT / "indicators.yaml"

OUT_PARQUET = WGI_ROOT / "data" / "processed" / f"wgi_country_year_{START_YEAR}_{END_YEAR}.parquet"
OUT_CSV = WGI_ROOT / "data" / "processed" / f"wgi_country_year_{START_YEAR}_{END_YEAR}.csv"
...
def load_indicator_registry(path: Path) -> Tuple[int, Dict[str, str]]:
    ...
    source_id = int(cfg.get("source_id", 3))
...
def main() -> None:
    ...
    cache_dir = WGI_ROOT / "data" / "raw" / "api_cache"
    client = WGIClient(WGIClientConfig(cache_dir=cache_dir))
    ...
    if not wide.empty:
        missing = wide.isna().mean().sort_values()
