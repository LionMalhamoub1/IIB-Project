"""
Plot data coverage by year for all official datasets in Social Disruptions/External databases.
Saves bar charts to Social Disruptions/External databases/coverage_plots/
"""
...
df = pd.read_csv(os.path.join(BASE, "Google Trends", "data", "processed",
...
df = pd.read_csv(os.path.join(BASE, "Google Trends", "data", "processed",
                               "google_trends_country_week_2017_2025.csv"),
                 parse_dates=["week"])
datasets["Google Trends"] = collect(df.groupby(df["week"].dt.year).size())
