# Merges GDELT event labels with the economic feature panel. Run before train_backtest.py.
...
def add_supplemental_features(merged: pd.DataFrame) -> pd.DataFrame:
    ...
    log.info("Added engineered features: covid_period, fx_trend_consistent, "
             "inflation_accel.")
    # country_protest_baseline / country_strike_baseline are computed per-fold
    # in train_backtest.py from training data only — computing them here would
    # leak future data across walk-forward folds.
...
def run() -> pd.DataFrame:
    ...
    # don't fillna label cols here — recomputed below once the full date grid is set.
