"""
export_m1.py
Generates all M1 (Markets) figures and saves them to final figures/M1/.

Outputs:
  m1_static_timelines_protest_7d.png    -- country probability timelines, static backtest
  m1_static_timelines_strike_7d.png
  m1_expanding_timelines_protest_7d.png -- country timelines with retraining boundaries
  m1_expanding_timelines_strike_7d.png
  m1_expanding_performance_protest_7d.png -- monthly ROC-AUC and BSS
  m1_expanding_performance_strike_7d.png
  m1_coefficients_protest_7d.png        -- feature coefficients averaged over folds
  m1_coefficients_strike_7d.png
  m1_metrics.csv                        -- all metrics for M1 (static + expanding)
"""
...
def fig_m1_static_timelines(target: str) -> None:
    ...
    if preds.empty:
        print(f"No M1 predictions in {target}")
        return
    ...
    fig.suptitle(
        f"M1 Markets — Predicted Probability Timelines\n"
    ...
    for ax, iso3 in zip(axes[:, 0], countries):
        ...
        ax.plot(c["date"], c["y_pred"], color=COLOR, lw=1.5, label="M1 Markets")
    ...
    plt.tight_layout()
    out = OUT_DIR / f"m1_static_timelines_{target}.png"
...
def fig_m1_expanding_timelines(target: str) -> None:
    ...
    preds = preds[preds["model_name"] == MODEL_NAME]
    if preds.empty:
        print(f"No M1 expanding predictions for {target}")
        return
    ...
    fig, axes = plt.subplots(n, 1, figsize=(14, 2.8 * n), squeeze=False)
    fig.suptitle(
        f"M1 Markets — Expanding Window Timelines\n"
    ...
    for ax, iso3 in zip(axes[:, 0], countries):
        ...
        if ax is axes[0, 0]:
            handles = [
                mpatches.Patch(color=COLOR, label="M1 Markets"),
    ...
    axes[-1, 0].set_xlabel("Date", fontsize=9)
    plt.tight_layout()
    out = OUT_DIR / f"m1_expanding_timelines_{target}.png"
    fig.savefig(out, dpi=150, bbox_inches="tight")
    plt.close(fig)
    print(f"Saved: {out}")
...
# ---------------------------------------------------------------------------
# Expanding window: monthly ROC-AUC and BSS
# ---------------------------------------------------------------------------

def fig_m1_expanding_performance(target: str) -> None:
    ...
    fig.suptitle(
        f"M1 Markets — Expanding Window Monthly Performance\n"
    ...
    plt.tight_layout()
    out = OUT_DIR / f"m1_expanding_performance_{target}.png"
    fig.savefig(out, dpi=150, bbox_inches="tight")
    plt.close(fig)
    print(f"Saved: {out}")
...
def fig_m1_coefficients(target: str) -> None:
...
def save_m1_metrics() -> None:
    ...
    if rows:
        ...
        for c in ["roc_auc", "pr_auc", "brier", "brier_skill_score"]:
            out[c] = out[c].round(4)
        out.to_csv(OUT_DIR / "m1_metrics.csv", index=False)
        print(f"Saved: {OUT_DIR / 'm1_metrics.csv'}  ({len(out)} rows)")
...
if __name__ == "__main__":
    import shutil
    print(f"Saving M1 outputs to {OUT_DIR}\n")
    for target in TARGETS:
        fig_m1_static_timelines(target)
        fig_m1_expanding_timelines(target)
        fig_m1_expanding_performance(target)
        fig_m1_coefficients(target)
    save_m1_metrics()

    shutil.copy2(__file__, OUT_DIR / "export_m1.py")
    print(f"Saved: {OUT_DIR / 'export_m1.py'}")
    print("\nDone.")
