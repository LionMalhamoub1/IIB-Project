...
fetch_fx_missing.py
===================
Downloads FX data for the 22 panel countries that were missing exchange-rate
features in the original markets pipeline (high-income / Eurozone countries
that were excluded from the original config).

Produces:
    v2/data/interim/fx_missing_countries.parquet
        Columns: country_iso3, date, fx_lcu_usd, fx_log_return,
                 fx_pct_7d, fx_pct_30d, fx_pct_90d, fx_vol_7d, fx_vol_30d

This file is consumed by build_panel.py to fill in the missing FX values
before the modelling panel is assembled.

Usage
-----
    python src/fetch_fx_missing.py          # fetch and save
    python src/fetch_fx_missing.py --check  # only print missingness summary
"""
