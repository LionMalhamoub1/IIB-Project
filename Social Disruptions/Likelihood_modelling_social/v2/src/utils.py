...
utils.py
========
Shared utilities for the GDELT-label modelling pipeline (v2).

Kept consistent with the original src/utils.py but with GDELT-specific
target helpers replacing the ACLED-based ones.
"""
...
def get_target_col(event_type: str, horizon: int) -> str:
    """
    Return the pre-computed GDELT target column name.

    Parameters
    ----------
    event_type : "protest" or "strike"
    horizon    : 7 or 30

    Returns
    -------
    Column name string, e.g. "protest_7d"
    """
...
def estimate_reliable_countries(
    ...
    Parameters
    ----------
    df                           : Panel DataFrame with country_iso3,
                                   n_articles, n_protest_events, n_strike_events.
    min_median_articles_per_event: Countries with median articles per event
                                   below this threshold are treated as unreliable.
                                   Default 2.0.
    """
...
def make_target_pu_country(
    ...
    event_type          : "protest" or "strike".
    horizon             : 7 or 30.
    reliable_countries  : Set of ISO3 codes from estimate_reliable_countries().
    """
...
def estimate_labelling_probability(
    ...
    Parameters
    ----------
    df          : Training fold DataFrame (must include coverage_flag and target col).
    event_type  : "protest" or "strike".
    horizon     : 7 or 30.
...
def make_target_gdelt(
    ...
    Parameters
    ----------
    df                   : The modelling panel DataFrame.
    event_type           : "protest" or "strike".
    horizon              : 7 or 30.
    exclude_low_coverage : If True, sets target to NaN for low-coverage country-days
                           (unreliable negative labels).  Recommended when training.
    """
    col = get_target_col(event_type, horizon)
    if col not in df.columns:
        raise KeyError(
