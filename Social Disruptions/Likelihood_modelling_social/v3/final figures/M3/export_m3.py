"""
export_m3.py
Generates all M3 (Structural) figures and saves them to final figures/M3/.

M3 = M0 + M1 + M2 + M3_ADD (Gini, COVID, baselines, commodity interactions)

Outputs:
  m3_static_timelines_{protest,strike}_7d.png
  m3_expanding_timelines_{protest,strike}_7d.png
  m3_expanding_performance_{protest,strike}_7d.png
  m3_coefficients_{protest,strike}_7d.png
  m3_metrics.csv
"""
...
def fig_m3_static_timelines(target: str) -> None:
    ...
    if preds.empty:
        print(f"No M3 predictions in {target}"); return
    ...
    fig.suptitle(
        f"M3 Structural — Predicted Probability Timelines\n"
    ...
    for ax, iso3 in zip(axes[:, 0], countries):
        ...
        ax.plot(c["date"], c["y_pred"], color=COLOR, lw=1.5, label="M3 Structural")
        events = c[c["y_true"] == 1]
        if not events.empty:
    ...
    plt.tight_layout()
    out = OUT_DIR / f"m3_static_timelines_{target}.png"
    fig.savefig(out, dpi=150, bbox_inches="tight")
    plt.close(fig)
    print(f"Saved: {out}")


def fig_m3_expanding_timelines(target: str) -> None:
    ...
    preds = preds[preds["model_name"] == MODEL_NAME]
    if preds.empty:
        print(f"No M3 expanding predictions for {target}"); return
    ...
    fig, axes = plt.subplots(n, 1, figsize=(14, 2.8 * n), squeeze=False)
    fig.suptitle(
        f"M3 Structural — Expanding Window Timelines\n"
    ...
    for ax, iso3 in zip(axes[:, 0], countries):
        ...
        if ax is axes[0, 0]:
            handles = [
                mpatches.Patch(color=COLOR, label="M3 Structural"),
    ...
    axes[-1, 0].set_xlabel("Date", fontsize=9)
    plt.tight_layout()
    out = OUT_DIR / f"m3_expanding_timelines_{target}.png"
    fig.savefig(out, dpi=150, bbox_inches="tight")
    plt.close(fig)
    print(f"Saved: {out}")


def fig_m3_expanding_performance(target: str) -> None:
    ...
    fig.suptitle(
        f"M3 Structural — Expanding Window Monthly Performance\n"
    ...
    plt.tight_layout()
    out = OUT_DIR / f"m3_expanding_performance_{target}.png"
    fig.savefig(out, dpi=150, bbox_inches="tight")
    plt.close(fig)
    print(f"Saved: {out}")
...
def fig_m3_coefficients(target: str) -> None:
    ...
    fig, ax = plt.subplots(figsize=(9, max(4, len(avg) * 0.35)))
    ...
    ax.set_title(
        f"M3 Structural — Feature Coefficients\n{TARGET_LABELS[target]} | averaged over folds",
        fontsize=11, fontweight="bold",
    )
    plt.tight_layout()
    out = OUT_DIR / f"m3_coefficients_{target}.png"
    fig.savefig(out, dpi=150, bbox_inches="tight")
    plt.close(fig)
    print(f"Saved: {out}")


def save_m3_metrics() -> None:
    rows = []
    ...
    if rows:
        ...
        for c in ["roc_auc", "pr_auc", "brier", "brier_skill_score"]:
            out[c] = out[c].round(4)
        out.to_csv(OUT_DIR / "m3_metrics.csv", index=False)
        print(f"Saved: {OUT_DIR / 'm3_metrics.csv'}  ({len(out)} rows)")


if __name__ == "__main__":
    import shutil
    print(f"Saving M3 outputs to {OUT_DIR}\n")
    for target in TARGETS:
        fig_m3_static_timelines(target)
        fig_m3_expanding_timelines(target)
        fig_m3_expanding_performance(target)
        fig_m3_coefficients(target)
    save_m3_metrics()
    shutil.copy2(__file__, OUT_DIR / "export_m3.py")
    print(f"Saved: {OUT_DIR / 'export_m3.py'}")
    print("\nDone.")
