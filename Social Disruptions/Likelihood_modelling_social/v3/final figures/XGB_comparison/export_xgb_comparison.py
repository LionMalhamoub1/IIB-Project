...
def fig_lr_vs_xgb_bss():
    ALL_LR = {
        ...
        "model2_full":        "M2 Full Macro",
        "model3_structural":  "M3 Structural",
        "model4_fao":         "M4 FAO+GTA",
        "model_lr_nolag":     "M5 No Lags",
    }
    ALL_XGB = {
        "model5_xgb":       "M6 XGB Full",
        "model6_xgb_nolag": "M7 XGB No Lags",
    }
    ...
    for ax, (target, title) in zip(axes, TARGETS):
        ...
        ax.text(lr_mid,  0.12, "Logistic Regression", ha="center", va="bottom",
                fontsize=9, fontweight="bold", color="#2166ac", transform=trans,
                bbox=dict(facecolor="white", edgecolor="none", alpha=0.65, pad=2))
        ax.text(xgb_mid, 0.12, "XGBoost",            ha="center", va="bottom",
                fontsize=9, fontweight="bold", color="#b04010", transform=trans,
                bbox=dict(facecolor="white", edgecolor="none", alpha=0.65, pad=2))
        ...
        ax.set_xticklabels(all_lbls, rotation=30, ha="right", fontsize=9)
