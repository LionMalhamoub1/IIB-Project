    ...
    for ax, target in zip(axes, TARGETS):
        ...
        for year, style in YEAR_STYLE.items():
            ...
            line, = ax.plot(range(len(MODELS)), vals,
                            ...
                            lw=2.0, marker="o", ms=7, label=style["label"],
                            zorder=3)
            if target == TARGETS[0]:
                handles.append(line)
        ...
        ax.set_xticklabels([SIMPLE_LABELS[m] for m in MODELS], fontsize=15)
        ax.tick_params(axis="y", labelsize=14)
        ax.set_ylabel("Brier Skill Score" if ax is axes[0] else "", fontsize=15)
        ax.set_title(TARGET_LABELS[target], fontsize=16, fontweight="bold")
        ax.spines["left"].set_linewidth(0.8)
        ax.spines["bottom"].set_linewidth(0.8)

    fig.legend(handles=handles, fontsize=14, loc="lower center",
