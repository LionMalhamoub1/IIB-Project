"""
export_m5.py
Generates all M5 (LR No Lags) figures and saves them to final figures/M5/.

M5 = model_lr_nolag = M1 + M2 + M3 + M4 (no GDELT autoregressive lags)
Pure early-warning model: no persistence signal, only macro/structural/market features.

Outputs:
  m5_static_timelines_{protest,strike}_7d.png
  m5_expanding_timelines_{protest,strike}_7d.png
  m5_expanding_performance_{protest,strike}_7d.png
  m5_coefficients_{protest,strike}_7d.png
  m5_metrics.csv
"""
...
def fig_m5_static_timelines(target: str) -> None:
    ...
    if preds.empty:
        print(f"No M5 predictions in {target}"); return
    ...
    fig.suptitle(
        f"M5 LR No Lags — Predicted Probability Timelines\n"
    ...
    for ax, iso3 in zip(axes[:, 0], countries):
        ...
        ax.plot(c["date"], c["y_pred"], color=COLOR, lw=1.5, label="M5 LR No Lags")
        events = c[c["y_true"] == 1]
        if not events.empty:
    ...
    plt.tight_layout()
    out = OUT_DIR / f"m5_static_timelines_{target}.png"
    fig.savefig(out, dpi=150, bbox_inches="tight")
    plt.close(fig)
    print(f"Saved: {out}")


def fig_m5_expanding_timelines(target: str) -> None:
    ...
    preds = preds[preds["model_name"] == MODEL_NAME]
    if preds.empty:
        print(f"No M5 expanding predictions for {target}"); return
    ...
    fig, axes = plt.subplots(n, 1, figsize=(14, 2.8 * n), squeeze=False)
    fig.suptitle(
        f"M5 LR No Lags — Expanding Window Timelines\n"
    ...
    for ax, iso3 in zip(axes[:, 0], countries):
        ...
        if ax is axes[0, 0]:
            handles = [
                mpatches.Patch(color=COLOR, label="M5 LR No Lags"),
    ...
    axes[-1, 0].set_xlabel("Date", fontsize=9)
    plt.tight_layout()
    out = OUT_DIR / f"m5_expanding_timelines_{target}.png"
    fig.savefig(out, dpi=150, bbox_inches="tight")
    plt.close(fig)
    print(f"Saved: {out}")


def fig_m5_expanding_performance(target: str) -> None:
    ...
    fig.suptitle(
        f"M5 LR No Lags — Expanding Window Monthly Performance\n"
    ...
    plt.tight_layout()
    out = OUT_DIR / f"m5_expanding_performance_{target}.png"
    fig.savefig(out, dpi=150, bbox_inches="tight")
    plt.close(fig)
    print(f"Saved: {out}")
...
def fig_m5_coefficients(target: str) -> None:
    ...
    ax.set_xlabel("Coefficient (standardised features)", fontsize=18)
    ax.tick_params(axis="y", labelsize=17)
    ax.tick_params(axis="x", labelsize=17)
    ax.set_title(
        f"M5 LR No Lags — Feature Coefficients\n{TARGET_LABELS[target]} | averaged over folds",
        fontsize=20, fontweight="bold",
    )
    plt.tight_layout()
    out = OUT_DIR / f"m5_coefficients_{target}.png"
    fig.savefig(out, dpi=150, bbox_inches="tight")
    plt.close(fig)
    print(f"Saved: {out}")


def save_m5_metrics() -> None:
    rows = []
    ...
    if rows:
        ...
        for c in ["roc_auc", "pr_auc", "brier", "brier_skill_score"]:
            out[c] = out[c].round(4)
        out.to_csv(OUT_DIR / "m5_metrics.csv", index=False)
        print(f"Saved: {OUT_DIR / 'm5_metrics.csv'}  ({len(out)} rows)")


if __name__ == "__main__":
    import shutil
    print(f"Saving M5 outputs to {OUT_DIR}\n")
    for target in TARGETS:
        fig_m5_static_timelines(target)
        fig_m5_expanding_timelines(target)
        fig_m5_expanding_performance(target)
        fig_m5_coefficients(target)
    save_m5_metrics()
    shutil.copy2(__file__, OUT_DIR / "export_m5.py")
    print(f"Saved: {OUT_DIR / 'export_m5.py'}")
    print("\nDone.")
