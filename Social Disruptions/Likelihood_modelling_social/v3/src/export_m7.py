"""
export_m7.py
Generates all M7 (XGBoost No Lags) figures and saves them to final figures/M7/.

M7 = model6_xgb_nolag — XGBoost without GDELT lag features (pure early-warning)

Outputs:
  m7_static_timelines_{protest,strike}_7d.png
  m7_expanding_timelines_{protest,strike}_7d.png
  m7_expanding_performance_{protest,strike}_7d.png
  m7_shap_{protest,strike}_7d.png
  m7_metrics.csv
"""
...
def fig_m7_static_timelines(target: str) -> None:
    ...
    if preds.empty:
        print(f"No M7 predictions in {target}"); return
    ...
    fig.suptitle(
        f"M7 XGBoost No Lags — Predicted Probability Timelines\n"
    ...
    for ax, iso3 in zip(axes[:, 0], countries):
        ...
        ax.plot(c["date"], c["y_pred"], color=COLOR, lw=1.5, label="M7 XGBoost No Lags")
        events = c[c["y_true"] == 1]
        if not events.empty:
    ...
    plt.tight_layout()
    out = OUT_DIR / f"m7_static_timelines_{target}.png"
    fig.savefig(out, dpi=150, bbox_inches="tight")
    plt.close(fig)
    print(f"Saved: {out}")


def fig_m7_expanding_timelines(target: str) -> None:
    ...
    preds = preds[preds["model_name"] == MODEL_NAME]
    if preds.empty:
        print(f"No M7 expanding predictions for {target}"); return
    ...
    fig, axes = plt.subplots(n, 1, figsize=(14, 2.8 * n), squeeze=False)
    fig.suptitle(
        f"M7 XGBoost No Lags — Expanding Window Timelines\n"
    ...
    for ax, iso3 in zip(axes[:, 0], countries):
        ...
        if ax is axes[0, 0]:
            handles = [
                mpatches.Patch(color=COLOR, label="M7 XGBoost No Lags"),
    ...
    axes[-1, 0].set_xlabel("Date", fontsize=9)
    plt.tight_layout()
    out = OUT_DIR / f"m7_expanding_timelines_{target}.png"
    fig.savefig(out, dpi=150, bbox_inches="tight")
    plt.close(fig)
    print(f"Saved: {out}")


def fig_m7_expanding_performance(target: str) -> None:
    ...
    fig.suptitle(
        f"M7 XGBoost No Lags — Expanding Window Monthly Performance\n"
    ...
    plt.tight_layout()
    out = OUT_DIR / f"m7_expanding_performance_{target}.png"
    fig.savefig(out, dpi=150, bbox_inches="tight")
    plt.close(fig)
    print(f"Saved: {out}")


def fig_m7_shap(target: str) -> None:
    ...
    ax.set_xlabel("Mean |SHAP value| (averaged over folds)", fontsize=18)
    ax.tick_params(axis="y", labelsize=17)
    ax.tick_params(axis="x", labelsize=17)
    ax.set_title(
        f"M7 XGBoost No Lags — Feature Importance (SHAP)\n"
    ...
    if has_sign:
        handles = [
            mpatches.Patch(color="#d62728", label="Increases P(event)"),
            mpatches.Patch(color="#1f77b4", label="Decreases P(event)"),
        ]
        ax.legend(handles=handles, fontsize=16, loc="lower right", frameon=False)

    plt.tight_layout()
    out = OUT_DIR / f"m7_shap_{target}.png"
    fig.savefig(out, dpi=150, bbox_inches="tight")
    plt.close(fig)
    print(f"Saved: {out}")


def save_m7_metrics() -> None:
    rows = []
    ...
    if rows:
        ...
        for c in ["roc_auc", "pr_auc", "brier", "brier_skill_score"]:
            out[c] = out[c].round(4)
        out.to_csv(OUT_DIR / "m7_metrics.csv", index=False)
        print(f"Saved: {OUT_DIR / 'm7_metrics.csv'}  ({len(out)} rows)")


if __name__ == "__main__":
    import shutil
    print(f"Saving M7 outputs to {OUT_DIR}\n")
    for target in TARGETS:
        fig_m7_static_timelines(target)
        fig_m7_expanding_timelines(target)
        fig_m7_expanding_performance(target)
        fig_m7_shap(target)
    save_m7_metrics()
    shutil.copy2(__file__, OUT_DIR / "export_m7.py")
    print(f"Saved: {OUT_DIR / 'export_m7.py'}")
    print("\nDone.")
