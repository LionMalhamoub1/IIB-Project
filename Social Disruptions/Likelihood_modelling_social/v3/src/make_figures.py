...
make_figures.py
===============
Generates dissertation figures from the GDELT modelling pipeline (v2).

Run after train_backtest.py has completed.

Standard figures (saved to v2/figures/):
  01_model_comparison.png
  02_pr_curves.png
  03_feature_importance_xgb.png
  04_calibration.png
  05_country_timeline.png
  06_lift_curves.png
  07_model_progression.png

Dissertation figures for best LR and best XGBoost model:
  lr_coefficients.png       LR coefficients by magnitude, coloured by block
  shap_values.png           Mean |SHAP| by feature, coloured by block
  calibration_curve.png     Reliability diagrams for best LR + best XGBoost
  cumulative_gain.png       Cumulative gain curves, both test folds
  country_timelines.png     Predicted probability timelines, 5 countries
  feature_importance.txt    Top-10 features per block, both models
"""
...
def main() -> None:
    ...
    # Copy final selection to final figures/
    import shutil
    final_dir = _V2 / "final figures"
    ...
    print(f"\nCopying {len(finals)} figures to final figures/...")
    for name in finals:
        src = FIG_DIR / name
        if src.exists():
