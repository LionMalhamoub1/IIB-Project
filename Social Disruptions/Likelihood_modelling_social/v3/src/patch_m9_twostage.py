"""
Patch: add model9_twostage to existing backtest outputs.

Stage 1 — structural risk (slow features: M2 + M3 + M4)
    LR without country FE; outputs a scalar 'structural_risk_score'.
    Trained with 5-fold stratified CV on training data so that Stage 2
    never sees in-sample Stage 1 predictions (avoids leakage).

Stage 2 — event trigger (fast features: M0 + M1 + structural_risk_score)
    LR with country FE; predicts final event probability given current
    conditions and the structural environment.

Feature importance splits naturally into:
    Stage 1 coefs  — which structural factors elevate background risk
    Stage 2 coefs  — which triggers fire, plus weight on structural_risk_score
    Total slow effect of feature f = coef1[f] * coef2['structural_risk_score']
"""
