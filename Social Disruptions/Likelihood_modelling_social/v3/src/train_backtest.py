...
train_backtest.py
=================
Walk-forward backtest using GDELT-derived event labels as targets.

Key differences from the original (v1) pipeline
------------------------------------------------
- Labels come from GDELT news clustering, not ACLED.
- Four targets: protest × {7d, 30d}  and  strike × {7d, 30d}.
- Persistence (M0) features use GDELT lag counts, not ACLED lag counts.
- Data window: 2018-2020  =>  1 test fold (train: 2018-2019, test: 2020).
- Low-coverage country-days are masked as NaN so the model never trains on
  unreliable zero labels (controlled by EXCLUDE_LOW_COVERAGE).

Model specifications
--------------------
  model0_persistence  GDELT lag features only (autoregressive baseline)
  model1_markets      + FX and oil market features
  model2_full         + macro, labour, governance indicators
  model4_fao          + FAO food price indices and GTA trade interventions
  model5_xgb          XGBoost on the full feature set (tuned via Optuna)
  model6_xgb_nolag    XGBoost without GDELT lag features (pure early-warning)
"""
...
# NOTE: protest_30d removed — a protest occurring anywhere in a country in 30 days
# is true ~94% of the time for reliably-monitored countries, leaving no variation
# for a classifier to discriminate.  The three remaining targets are meaningful.
...
APPLY_VIF_FILTER: bool  = True   # iteratively drop highest-VIF feature until all VIFs < threshold
VIF_THRESHOLD:    float = 10.0   # standard threshold; lower (e.g. 5) is more aggressive
...
def run_lr_fold(
    ...
    if APPLY_VIF_FILTER:
        feat, _ = vif_filter(train_clean, feat, VIF_THRESHOLD)
        if not feat:
            logger.warning("VIF filter removed all features for %s fold=%d.", model_name, fold_id)
            return pd.DataFrame(), pd.DataFrame(), {}
