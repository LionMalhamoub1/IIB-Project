"""
train_expanding_lr.py
=====================
Monthly expanding-window backtest for Logistic Regression models (M0-M4).

Starting from a 2017-2019 base, retrains on the 1st of each month,
incorporating all observed data up to that point. Covers 2020-2021.

Outputs (v2/data/processed/expanding_lr/):
  preds_{target}.parquet       daily predictions for all months concatenated
  metrics_{target}.csv         per-month ROC-AUC, PR-AUC, Brier, BSS per model

Figures (v2/figures/):
  expanding_lr_timelines.png   probability time-series, 5 countries
  expanding_lr_performance.png monthly metrics for M0-M4
"""
...
from utils import compute_metrics, make_target_gdelt, vif_filter
...
def run_expanding_lr(panel: pd.DataFrame) -> None:
    ...
    for event_type, horizon in TARGETS:
        ...
        # Pre-compute VIF-filtered feature sets once on base training data.
        # Collinearity is structural and stable across months, so there is no
        # need to re-run the filter (which is slow) for every monthly period.
        vif_filtered_feats: dict[str, list[str]] = {}
        if APPLY_VIF_FILTER:
            base_train = df[(df["date"] >= TRAIN_START) &
                            (df["date"] <= BASE_TRAIN_END)].dropna(subset=["y"])
            log.info("Pre-computing VIF filter on base training data (2017-2019)...")
            for model_name, feat_list in FEATURE_SETS.items():
                feats_candidate = [f for f in feat_list
                                   if f in df.columns or f in BASELINE_COLS]
                feats_candidate = [f for f in feats_candidate
                                   if f in base_train.columns]
                filtered, _ = vif_filter(base_train, feats_candidate, VIF_THRESHOLD)
                vif_filtered_feats[model_name] = filtered
                log.info("  %s: %d -> %d features after VIF",
                         model_name, len(feats_candidate), len(filtered))
        ...
        for train_cutoff, month_start, month_end in periods:
            ...
            for model_name, feat_list in FEATURE_SETS.items():
                # Use pre-computed VIF-filtered features; fall back to full list
                if APPLY_VIF_FILTER and model_name in vif_filtered_feats:
                    feats = [f for f in vif_filtered_feats[model_name]
                             if f in train_labeled.columns]
                else:
                    feats = [f for f in feat_list
                             if f in df.columns or f in BASELINE_COLS]
