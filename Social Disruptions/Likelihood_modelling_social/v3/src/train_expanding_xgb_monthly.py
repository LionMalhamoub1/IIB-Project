"""
train_expanding_xgb_monthly.py
Monthly expanding window evaluation for XGBoost models.

8 of the 24 monthly training cutoffs coincide with the quarterly cutoffs already
computed by train_expanding_xgb.py.  For those months (Jan/Apr/Jul/Oct each year)
predictions are extracted directly from the existing quarterly parquet — no retrain.
The remaining 16 months are newly fitted.

Outputs saved to data/processed/expanding_xgb_monthly/:
  preds_{target}.parquet   (column: retrain_month = "2020-01", "2020-02", ...)
  metrics_{target}.csv
"""
