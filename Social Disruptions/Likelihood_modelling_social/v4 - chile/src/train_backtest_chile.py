"""
train_backtest_chile.py
=======================
Chile-only version of the v3 static backtest.

Filters the 39-country GDELT panel to Chile (CHL) only and re-runs
all models M0-M7 with country fixed-effects disabled.
Useful for comparing single-country vs pooled-panel performance.

Key differences from v3/train_backtest.py
------------------------------------------
- COUNTRY_FILTER = "CHL"        — only Chile rows
- INCLUDE_COUNTRY_FE = False    — meaningless with one country
- APPLY_VIF_FILTER  = False     — many features are constant for a
                                   single country; VIF becomes unreliable
- XGB_OPTUNA_TRIALS = 20        — smaller dataset, fewer trials needed
- No income-group / nobaseline variants (not relevant for one country)
- Output → v4 - chile/data/processed/
"""
