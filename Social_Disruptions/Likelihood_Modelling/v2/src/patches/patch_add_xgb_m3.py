# Adds model_xgb_m3 (XGBoost on M0+M1+M2+M3) to the monthly expanding window.
...
def run() -> None:
    ...
    for event_type, horizon in TARGETS:
        ...
        if q_path.exists():
            ...
            # quarterly preds are for model5_xgb — model_xgb_m3 has a different
            # feature set, so its predictions can't be reused unless it already ran quarterly.
